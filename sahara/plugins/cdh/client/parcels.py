...
def _get_parcel(resource_root, path):
    return types.call(resource_root.get, path, ApiParcel, api_version=3)
...
def get_all_parcels(resource_root, cluster_name="default", view=None):
    ...
    @since: API v3
    """
    return types.call(resource_root.get, PARCELS_PATH % (cluster_name,),
        ApiParcel, True, params=view and dict(view=view) or None, api_version=3)


class ApiParcelState(types.BaseApiObject):
    """
    An object that represents the state of a parcel.
    """
    _ATTRIBUTES = {
        'progress'      : types.ROAttr(),
        'totalProgress' : types.ROAttr(),
        'count'         : types.ROAttr(),
        'totalCount'    : types.ROAttr(),
        'warnings'      : types.ROAttr(),
        'errors'        : types.ROAttr(),
      }
...
class ApiParcel(types.BaseApiResource):
    """
    ...
    """
    _ATTRIBUTES = {
      'product'     : types.ROAttr(),
      'version'     : types.ROAttr(),
      'stage'       : types.ROAttr(),
      'state'       : types.ROAttr(ApiParcelState),
